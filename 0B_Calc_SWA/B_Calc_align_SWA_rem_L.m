...
for i=1:nfiles
    ...
    if i==1  
       ...
       %adapt (only first 12 hours there), shift data according to starttime 14h
        ...
        all_rem=zeros(1,length(vissymbplot));
        all_rem(find(vissymbplot=='rem'))=1;
        artndxn_cut_0=all_rem;
...
cd(savepath)
save('SWA_scoring_episodes_rem','SWA_normalized','SWA_absolute','scoring','filenames','episodes')
close all  
clear
...
SWA_rem=SWA_absolute;

load('SWA_scoring_episodes.mat')

%check if it gives same result for nrem sleep as SWA_normalized

for i = 1:11
    
    SWA_nrem_norm(i,:) = SWA_abslolute(i,:)/nanmean(SWA_absolute(i,63*900+1:87*900));

end


%normalize SWA rem with mean SWA NREM sleep

for i = 1:11
    
    SWA_rem_norm(i,:) = SWA_rem(i,:)/nanmean(SWA_absolute(i,63*900+1:87*900));

end

for i = 1:11
    
minimum_SWA(i) = prctile(SWA_rem_norm(i,:),1)
...
maximum_SWA(i) = max(rmoutliers(SWA_nrem_norm(i,:)))
