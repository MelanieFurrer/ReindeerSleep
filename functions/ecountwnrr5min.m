function [wakdura,nremdura,remdura,rumdura,ba,baa]=ecountwnrr5min(STD)
...
while epoch<d
    ...
    if STD(epoch)=='n' | STD(epoch)=='2'
        ...
        while epoch<d & abort==0
            ...
            before=STD(epoch);
            if xcount>=74 & no_nrem(xcount)-no_nrem(xcount-73)==73  % max. 5min breaks allowed
                abort=1;
            elseif xcount>900 % in total not more than 60 min breaks
                abort=1;
            end
        end 
...
while epoch<d
    ...
    if STD(epoch)=='t' | STD(epoch)=='4'
        ...
        while epoch<d & abort==0
            ...
            before=STD(epoch);
            if xcount>29 & no_rum(xcount)-no_rum(xcount-28)==28 % max. 2 min breaks allowed
                abort=1;
            elseif rcount>7 & rem(rcount)-rem(rcount-6)==6 % if there is at least 30 sec rem, episode is also terminated
                abort=1;
            elseif xcount>449  % in total not more than 10 min breaks
                abort=1;
            end
        end 
        rumdur=(stope+1)-starte;
        if rumdur>14 & after~='a' % save only episodes that are at least 2 min long
